...
### for
```java
Quando executamos uma sequência de comandos na linguagem Java e precisamos que ela se repita até determinada condição seja contemplada
chamamos isso de "loop". O loop For, nesse caso, é responsável por iterar alguma parte de nosso código quando precisamos, não sendo
necessário reescrevê-lo, automatizando dessa maneira, qualquer incremento ou decremento de uma variável.
